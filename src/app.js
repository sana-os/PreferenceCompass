(() => {
 ...
 function progress(){const n=count();$('progressCount').textContent=n+' / '+Q.length;$('progressBar').style.width=n+'%';$('progressTrack').setAttribute('aria-valuenow',String(n));
  $('categoryNav').innerHTML=D.categories.map((c,i)=>{const done=c.questions.filter(q=>C.status(state.answers[q.id])!=='unanswered').length;return '<button type="button" class="category-button'+(!review&&Q[current].categoryId===c.id?' active':'')+'" data-category="'+i+'"'+(!review&&Q[current].categoryId===c.id?' aria-current="step"':'')+'><span class="num" dir="ltr">'+String(i+1).padStart(2,'0')+'</span><span class="name">'+esc(c.name)+'</span><span class="done" dir="ltr">'+done+'/10</span></button>';}).join('');
 ...
 function showReview(){review=true;$('questionView').hidden=true;$('reviewView').hidden=false;const n=count();$('reviewCompletion').textContent=n+' / '+Q.length;$('unansweredSummary').textContent=n===100?t('complete',{total:100}):t('remaining',{n:100-n});$('firstUnanswered').disabled=n===100;$('downloadMarkdown').disabled=n!==100;$('downloadJson').disabled=n!==100;
  $('reviewList').innerHTML=D.categories.map(c=>'<details class="review-category"><summary>'+esc(c.name)+'</summary>'+c.questions.map(q=>{const a=state.answers[q.id]||{},s=C.status(a);return '<article class="answer-row"><h3><bdi dir="ltr">'+esc(q.id)+'</bdi> · '+esc(q.text)+'</h3><p><strong>'+esc(t(s))+'</strong>'+ (Number.isInteger(a.optionIndex)?' — '+esc(q.options[a.optionIndex].label):'')+'</p>'+ (a.freeText?'<p class="answer-note" dir="auto">'+esc(a.freeText)+'</p><p class="meta">'+esc(t('noteLanguage'))+': '+esc(a.freeTextLanguage||'und')+'</p>':'')+'<button class="secondary-button" type="button" data-edit="'+q.id+'">'+esc(t('edit'))+'</button></article>';}).join('')+'</details>').join('');
 ...
 document.addEventListener('keydown',e=>{if(review||$('resetDialog').open||e.ctrlKey||e.metaKey||e.altKey||e.target.matches('input,textarea,button,select,a,[contenteditable]'))return;if(['1','2','3','4'].includes(e.key)){update([{questionId:Q[current].id,optionIndex:Number(e.key)-1}]);go(current);}if(e.key===(D.language==='ar'?'ArrowRight':'ArrowLeft')){e.preventDefault();go(current-1,true);}if(e.key===(D.language==='ar'?'ArrowLeft':'ArrowRight')){e.preventDefault();next();}});
